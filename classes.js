  ...
  constructor(first_name, last_name, email, age, reports, title){
   ...
   updateTitle(){
     ...
     if (repLength === 0 ){
      this.title = 'Not a manager'
      return this
    }else if (repLength > 0 && repLength< 4){
      this.title = 'Barely Manager'
      
    }else if (repLength > 3 && repLength < 11){
      this.title = 'Mostly Manager'
    }else if (repLength > 10 && repLength < 51){
      this.title = 'Manager'
    }else if (repLength > 50 && repLength < 101){
...
class Machine{
  ...
  makeWidgets(num){
    ...
    let wear_and_tear = Math.floor(num / 50)
    this.wear_and_tear_count += wear_and_tear
  }
  fixMachine(){
